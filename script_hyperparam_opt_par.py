...
HyperparamOptManager = libs.hyperparam_opt.DistributedHyperparamOptManager
ModelClass = libs.tft_model.TemporalFusionTransformer


def main(expt_name, use_gpu, restart_opt, model_folder, hyperparam_iterations,
         worker_number, data_csv_path, data_formatter, num_iterations_per_worker=15, use_testing_mode = True):
  """Runs main hyperparameter optimization routine.
    Args:
      ...
      worker_number: Worker index definining which set of hyperparameters to test.
      data_csv_path: Path to csv file containing data
      data_formatter: Dataset-specific data fromatter (see
        expt_settings.dataformatter.GenericDataFormatter)
      num_iterations_per_worker: amount of jobs pro worker
  """
  ...
  if use_gpu:
    tf_config = utils.get_default_tensorflow_config(tf_device="gpu", gpu_id=int(worker_number)-1)
  ...
  opt_manager = HyperparamOptManager(param_ranges, fixed_params, model_folder, worker_number,
                                     search_iterations=hyperparam_iterations,
  ...
  while len(opt_manager.results.columns) < hyperparam_iterations/num_iterations_per_worker:
...
if __name__ == "__main__":
  ...
  def get_args():
    ...
    parser.add_argument(
        ...
        nargs="?",
        default=0,
        help="Index of a particular worker.")
    parser.add_argument(
  ...
  name, folder, use_tensorflow_with_gpu, restart, worker = get_args()
  ...
  main(
      ...
      use_testing_mode=False # change for real use
  )
